...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"𝙳𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"𝐊ʏα 𝐎яƉɛя 𝐇αι 𝐌αяɛ [𝐌αƨтɛя](https://t.me/llSANATANI_XDII)")
...
async def stop(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`𝚂𝚃𝙾𝙿𝙸𝙽𝙶 𝙳𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __ᴀᴅᴅɪɴɢ 𝙳𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼 ʙᴏᴛ sᴜᴅᴏ....__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"ᴛʜɪꜱ ᴜꜱᴇʀ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀ 𝙳𝙰𝚁𝙺 𝚇 𝚂𝙿𝙰𝙼 ʙᴏᴛ ꜱᴜᴅᴏ ᴜꜱᴇʀ !!")
        else:
            if len(sudousers) > 0:
